...
def main(dataset_name, confidence_threshold ,random_state ,lm_type ,layer ,attention_mechanism):
    ...
    class_word_similarity_file = open('../data/datasets/20News/class_word_similarity.txt', 'w',encoding='utf-8')
    document_keywords_file = open('../data/datasets/20News/document_keywords.txt', 'w', encoding='utf-8')
    bitem_doc_fre_file = open('../data/datasets/20News/model/bitem_doc_frequency.txt', 'w', encoding='utf-8')
    ...
    os.system('python indexDocs.py ../data/datasets/20News/document_keywords.txt ../data/datasets/20News/doc_wids.txt ../data/datasets/20News/voca.txt')
    all_document_words=[]

    vocab_file = open('../data/datasets/20News/voca.txt','r')
    ...
    doc_to_class_similarity = [[np.ones(5)] for i in range(len(tokenization_info))]
    t=0
    for document in document_words:
    ...
    #         bitem_class_similarity.append(np.array(bitem_similarities[0]))
    #     t = t+1
    # for t in range(len(bitem_class_similarity)):
    #     bitem_class_similarity[t] = MinmaxNormalization(bitem_class_similarity[t])
    ...
    for m in range(len(documet_token_cls_ids)):
        word_id = [0 for t in range(5)]
        for id in documet_token_cls_ids[m]:
            word_id[id] = word_id[id] + 1
        doc_word_to_class.append(np.array(word_id))

    for e in range(20):
        print(os.system('../src/btm est 5 '+str(len(vocab_lines))+' '+'10 0.01 50 100 ../data/datasets/20News/doc_wids.txt ../data/datasets/20News/model/ ../data/datasets/20News/class_word_similarity.txt '+str(bitem_class_similarity.shape[0])))
        print(os.system('../src/btm inf sum_b 5 ../data/datasets/20News/doc_wids.txt ../data/datasets/20News/model/ ../data/datasets/20News/model/bitem_doc_frequency.txt'))
        cosine_similarities = np.loadtxt('../data/datasets/20News/model/k5.pz_d')

        # repr_prediction = []
        # for doc_bitem in doc_to_class_similarity:
        #     repr_prediction.append(list(np.array(sum(doc_bitem))).index(max(np.array(sum(doc_bitem)))))
        # cosine_similarities = []
        # for doc_bitem in doc_to_class_similarity:
        #     cosine_similarities.append(sum(doc_bitem))
        repr_probility = np.array(cosine_similarities)
        ...
        pwz_list = np.loadtxt('../data/datasets/20News/model/k5.pw_z', dtype=np.float)
        pwz = []
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_name", default="20News")
